// netlify/functions/generate-horoscope.js
// Updated Netlify serverless function with personalization + biorhythm
...
exports.handler = async (event) => {
  ...
  try {
    const { sign, timeframe = 'daily', birthDate = null, biorhythm = null } = JSON.parse(event.body);

    if (!sign || !sign.name) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Sign information is required' })
      };
    }
